...
public:
  ...
  bool Init(const nr_local_addr& local_addr) {
    ...
    if (r) {
      MOZ_MTLOG(PR_LOG_ERROR, "Error formating interface address string.");
      return false;
    }
    key_ = buf;
  ...
  bool operator<(const LocalAddress& rhs) const {
    ...
    if (is_vpn_ != rhs.is_vpn_) {
      return !is_vpn_; // If |this| instance is not in VPN, |rhs| must be in
                       // VPN, we return true to make |this| sorted prior to
                       // |rhs|.
    }
