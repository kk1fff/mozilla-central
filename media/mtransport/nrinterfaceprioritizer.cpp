...
static int get_priority(void *obj, char *key, UCHAR *pref) {
  InterfacePrioritizer *ip = static_cast<InterfacePrioritizer*>(obj);
...
nr_interface_priority* CreateIntefacePrioritizer() {
...
nr_interface_prioritizer* CreateIntefacePrioritizer() {
  nr_interface_prioritizer *ip;
  int r = nr_interface_prioritizer_create_int(nullptr, &priorizer_vtbl, &ip);
  if (r != 0) {
    return nullptr;
  }
  return ip;
