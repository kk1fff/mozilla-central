...
class PrioritizerTest : public ::testing::Test {
 public:
  ...
  void AddInterface(const std::string& num, int type, int estimated_speed) {
    ...
    nr_ip4_str_port_to_transport_addr((sockaddr*)&addr, sizeof(sockaddr_in),
                                      IPPROTO_UDP, 0, &(local_addr.addr));
    strncpy(local_addr.addr.ifname, ifname.c_str(), MAXIFNAME);

    int r = nr_interface_prioritizer_add_interface(prioritizer_, &local_addr);
    ASSERT_EQ(r, 0);
...
TEST_F(PrioritizerTest, TestPrioritizer) {
  SetPriorizer(::mozilla::CreateIntefacePrioritizer());
